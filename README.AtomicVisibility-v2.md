...
1. Apply the patch (patch/0001-Add-atomic-visibility-feature-v2.patch) that postgres_fdw_plus provides to PostgreSQL, and build the patched PostgreSQL. For example,

   ```
   $ git clone -b master --depth 1 https://github.com/postgres/postgres.git
   $ cd postgres
   $ git am 0001-Add-atomic-visibility-feature-v2.patch
   $ ./configure --prefix=/opt/pgsql
   $ make
   $ make install
   ```

2. Create the database cluster and start the PostgreSQL server. For example,
